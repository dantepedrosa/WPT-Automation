...
def FEMM_results(path, femmfile, defaultCurrent):
#def FEMM_results():
    # Open document ----------------------------------------------
    femm.openfemm(1)
    ...
    femm.mi_setcurrent("Primary", defaultCurrent)
    femm.mi_setcurrent("Secondary", 0)
    femm.mi_analyze(1)

    femm.mi_loadsolution()
    primaryProps = femm.mo_getcircuitproperties("Primary")
    primaryInductance = abs(primaryProps[2]/primaryProps[0])
    primaryResistance = abs(primaryProps[1]/primaryProps[0])
    secondaryProps = femm.mo_getcircuitproperties("Secondary")
    mutualInductance = abs(secondaryProps[2]/primaryProps[0])
    femm.mo_close()

    femm.mi_setcurrent("Primary", 0)
    femm.mi_setcurrent("Secondary", defaultCurrent)
    femm.mi_analyze(1)

    femm.mi_loadsolution()
    secondaryProps = femm.mo_getcircuitproperties("Secondary")
    secondaryInductance = abs(secondaryProps[2]/secondaryProps[0])
    secondaryResistance = abs(secondaryProps[1]/secondaryProps[0])
    femm.mo_close()
    ...
    femm.mi_loadsolution()
    primaryProps = femm.mo_getcircuitproperties("Primary")
    coreResistance = abs((primaryProps[2]/primaryProps[0]).imag) * 2 * 60 * 3.1415926
    femm.mo_close()
...
def getCircuitProps(): 
    ...
    return dir_path, filename, fileExt
...
def get_efficiency(parameterList):
    ...
    f.write("Vcc Vs 0 SINE(0 6 60 0 0 0)\n")
    ...
    f.write(".meas RIN PARAM VIN/IIN\n")
    ...
    with open(logfile) as fd:
        ...
        for line in fd:
            ...
            if match:
                ...
                print("")
                print("The efficiency of the circuit is " + efficiency)
                print("")
    return efficiency, primary_capacitance

# MAIN
def main(current):

    dirpath, fileName, extension = getCircuitProps()
    if extension == "FEMM":
        circuitProps = FEMM_results(dirpath, fileName, current)
        
    else:
        fileNotFound = 2/0

    circuitEfficiency, capacitance = get_efficiency(circuitProps)
    ...
    return circuitProps, circuitEfficiency, capacitance
